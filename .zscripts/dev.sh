...
# Ensure DATABASE_URL points to the project database (guard against stale
# sandbox-injected file: URLs). Respects a real postgres URL if provided.
if [[ -z "${DATABASE_URL:-}" || "${DATABASE_URL:-}" == file:* ]]; then
        ENV_DB_URL=$(grep -E '^DATABASE_URL=' .env 2>/dev/null | head -1 | sed -E 's/^DATABASE_URL=//; s/^"//; s/"$//; s/^'"'"'//; s/'"'"'$//')
        if [ -n "$ENV_DB_URL" ]; then
                echo "[ENV] Using DATABASE_URL from .env (overriding '${DATABASE_URL:-<unset>}')"
